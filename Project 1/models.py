...
class NearEarthObject:
    ...
    @property
    def get_neo_designation(self):
        """Return neo designation attribute."""
        return self.designation

    @property
    def get_neo_name(self):
        """Return neo name attribute."""
        return self.name

    def __str__(self):
        """Return `str(self)`."""
        return f"A NearEarthObject {self.name} has a diameter of {self.diameter: .3f} km and \
                {'is' if self.hazardous == True else 'is not'} potentially hazardous"
...
class CloseApproach:
    ...
    def __str__(self):
        """Return `str(self)`."""
        return f"At {datetime_to_str(self.time)}, '{self.neo.fullname}' approaches Earth at a distance of {self.distance:.2f} au \
